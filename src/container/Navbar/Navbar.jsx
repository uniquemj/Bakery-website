...
const Navbar = () => {
  ...
  return (
    <>
        {/* Desktop and tablet */}
        <div className = "absolute z-50 hidden md:flex grid-row-3 justify-between items-center w-full py-4 px-[3.5rem] bg-[rgba(0,0,0,0.2)]">
            ...
            <motion.ul initial = {{opacity: 0, x : 200}}
             ...
             className = "hidden md:flex  items-center md:gap-8 lg:gap-15">
                ...
                <a href = "/#menu" className ="text-base text-navColor hover:text-headingColor duration-100 transition-all ease-in-out cursor-pointer">Menu</a>
                <Link to = "/about-us/" className ="text-base text-navColor hover:text-headingColor duration-100 transition-all ease-in-out cursor-pointer">About us</Link>
                <Link to = "/contact/" className ="text-base text-navColor hover:text-headingColor duration-100 transition-all ease-in-out cursor-pointer">Contact</Link>
            </motion.ul>
                    ...
                    <motion.img whileTap={{scale:0.6}} src={Avatar} alt="userprofile" className = "drop-shadow-xl" onClick = {displayMenu}/>
                    {
                        isMenu && (
                            <motion.div initial = {{opacity: 0, scale: 0.6}} animate = {{opacity: 1, scale: 1}} exit = {{opacity: 0, scale: 0.6}}className='w-40 right-12 bg-primary shadow-xl rounded-lg flex flex-col absolute top-[4.5rem] px-4 py-2'>
                                <Link to = "#Order">
                                    <p className='px-4 py-2 flex items-center gap-3 cursor-pointer hover:bg-slate-100 transition-all duration-100 ease-in-out text-textColor text-base' onClick = {()=>setIsMenu(false)}>Orders</p>
                                </Link>
                                <Link to = "/login/">
        ...
        {/* Mobile */}
        <div className = "absolute z-50 flex md:hidden justify-between items-center w-full py-4 px-10 bg-[rgba(0,0,0,0.2)]">
            <Link to ='/' className = "flex items-center">
            ...
            <motion.div initial = {{opacity: 0, scale: 0.6}} animate = {{opacity: 1, scale: 1}} exit = {{opacity: 0, scale: 0.6}} className='absolute bg-[rgba(0,0,0,0.9)] w-full h-[40rem] left-0 top-[4rem] text-black flex justify-center items-center flex-col'>
                <ul className = "flex flex-col justify-center items-center gap-4 md:gap-8 lg:gap-15 text-black">
                    ...
                    <Link to = "/about-us/" className ="text-base text-navColor hover:text-headingColor duration-100 transition-all ease-in-out cursor-pointer">About us</Link>
                    <Link to = "/contact/" className ="text-base text-navColor hover:text-headingColor duration-100 transition-all ease-in-out cursor-pointer">Contact</Link>
                    <p onClick={showCart} className ="text-base text-navColor hover:text-headingColor duration-100 transition-all ease-in-out cursor-pointer">Cart</p>
                </ul>
